...
class IDDADataset(VisionDataset):
    ...
    def __getitem__(self, index: int) -> Any: #called when used []
        ...
        # print(f'image_shape_idda = {image.shape}')
        # print(f'label_shape_idda = {label.shape}')

        # plt.imshow(image.permute(1, 2, 0))
        # plt.savefig('img_idda')
        # plt.imshow(label)
        # plt.savefig('label_tocca_capire')

        # transform = transforms.ToTensor()

        # image = transform(image)
        # label = transform(target)
        # print(f'image get_item tensor shape = {image.shape}')
        # print(f'label get_item tensor shape = {label.shape}')
        # plt.imshow(image.permute(1, 2, 0))
        # plt.savefig('img_tocca_capire')
        # plt.imshow(label)
        # plt.savefig('label_tocca_capire')

        # image = Image.open(self.root+'/images/'+self.list_samples[index]+'.jpg').convert("RGB")  # Open the image using PIL and convert to RGB
        # label = Image.open(self.root+"/labels/"+self.list_samples[index]+".png").convert("L")
