...
def create_clients(args, train_data, test_data, model, num_gpu, train,
    ...
    for i, user in enumerate(users):
        ...
        client = client_func(user, ds, model, args, batch_size, num_gpu, ckpt_path=ckpt_path, name=args.name, disable_ddp=disable_ddp)
...
def setup_clients(args, logger, writer, model, rank, num_gpu, device=None, ckpt_path=None,
                  disable_ddp=False):
    if args.dataset == 'cityscapes':
    ...
    train_clients = create_clients(args, train_data, test_data, model, rank, device, logger, writer,
                                   num_gpu, train=True, ckpt_path=ckpt_path, disable_ddp=disable_ddp)

    test_clients = create_clients(args, train_data, test_data, model, rank, device, logger, writer, num_gpu,
